...
```
LSD: LEAST-SQUARES METHODS TO ESTIMATE RATES AND DATES - v1.7
...
SYNOPSIS
        ./lsd [-i inputFile] [-d inputDateFile] [-o outputFile] [
-c] [-v mode] [-s sequenceLength] [-g outgroupFile] [
-f nbSamplings] [-e Zscore]

OPTIONS
        -a rootDate
           If the dates of all tips are equal (which is given by option -z), you must use this option to provide the root date.
           In this case, the input date file can be omitted, and the program estimates only the relative dates based on the given
           root date and tips date. By default, T[root]=0 and T[tips]=1.
        -b varianceParameter
        ...
        -c 
           By using this option, we impose the constraints that the date of every node is equal or smaller then
           the dates of its descendants. Without constraints, the runtime is linear (LD). With constraints, the
           problem is a quadratic programming and is solved efficiently (quasi-linear) by the active-set method.
        -d inputDateFile
           ...
           For example, if the input tree has 4 taxa a,b,c,d, and an internal node named n, then following is a possible date file:
            ...
            n l(2004.3)
           If this option is omitted, the program will estimate relative dates by giving T[root]=0 and T[tips]=1.
        -D outDateFormat
        ...
        -g outgroupFile
           If your data contain outgroups, then specify the name of the outgroup file here.
           The program will use the outgroups to root the trees. It will keep the outgroup in the trees if option -k is used with, 
           otherwise it will remove the outgroups. The format of this file should be:
                n
                OUTGROUP1
                OUTGROUP2
                ...
        ...
        -k
           Use this option to keep the outgroups (given in option -g) in the estimated tree. The root position is then estimated on the
           branch determined by the outgroups. If this option is not used, the outgroups will be removed.
        -l nullBlen
        ...
        -v variance
           Use this option if you want to apply variances for the branch lengths in order to recompense big errors on long estimated branch lengths. 
           The variance of the branch Bi is Vi = (Bi+b) where b is specified by option -b.
           If variance=1, then LSD uses the input branch lengths to calculate variances. If variance=2, then LSD
           runs twice where the second time it calculates the variances based on the estimated branch lengths of the first run.
        -V 
        ...
        -z tipsDate
           This option is used to give the date of the tips when they are all equal. It must be used with option -a to give the
           root date. In this case the input date file can be omitted, and the program estimates only the relative dates based on
           the given root date and tips date. By default, T[root]=0 and T[tips]=1.
```
